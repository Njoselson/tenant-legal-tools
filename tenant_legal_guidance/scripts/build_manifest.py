        ...
        try:
            ...
            if relevance_filter:
                ...
                if filter_result.is_relevant:
                    ...
                    logger.info(
                        f"✗ NOT RELEVANT: {case.case_name} - {filter_result.reason}"
                    )
...
def main():
    ...
    input_group.add_argument(
        "--justia",
        type=Path,
        help="Path to text file with Justia case URLs (one per line)"
    )
    input_group.add_argument(
        "--justia-search",
        action="store_true",
    ...
    parser.add_argument(
        "--court",
        help="Court filter for search (e.g., 'housing court')"
    )
    
    parser.add_argument(
        "--years",
        help="Year range for search (e.g., '2020-2025')"
    )
    ...
    parser.add_argument(
        "--deepseek-key",
        help="DeepSeek API key for LLM filtering"
    )
    ...
    try:
        ...
        if args.justia_search:
            ...
            if args.years:
                try:
                    ...
                    logger.error(f"Invalid --years format: {args.years}. Use format like '2020-2025'")
                    return 1
            ...
            stats = asyncio.run(build_justia_manifest(
                seed_urls=seed_urls,
        ...
        # Mode 2: Build manifest from Justia URLs file
            ...
            with args.justia.open("r") as f:
                seed_urls = [line.strip() for line in f if line.strip() and line.strip().startswith("http")]
