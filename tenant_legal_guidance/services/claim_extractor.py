...
class ClaimExtractor:
    ...
    async def extract_evidence_for_claim(
        ...
        try:
            ...
            self.logger.info(f"Extracted {len(evidence_list)} evidence items for claim '{claim.name}'")
    ...
    async def extract_full_proof_chain(
        ...
        for dmg in result.damages:
            if dmg.linked_outcome_id:
                result.relationships.append({
                    "source_id": dmg.linked_outcome_id,
                    "target_id": dmg.id,
                    "type": "IMPLY"
                })
    ...
    def _parse_claim_data(self, data: dict, doc_id: str, index: int) -> ExtractedClaim | None:
        ...
        try:
            ...
            return ExtractedClaim(
                ...
                claim_type=data.get("claim_type", data.get("claim_type_id")),  # Support both for migration
    ...
    def _parse_evidence_data(self, data: dict, claim_id: str, index: int, doc_id: str = "") -> ExtractedEvidence | None:
    ...
    def _parse_damages_data(
        ...
        try:
            ...
            for outcome in outcomes:
                if outcome.name.lower() in linked_outcome_name.lower() or \
                   linked_outcome_name.lower() in outcome.name.lower():
    ...
    async def extract_full_proof_chain_single(
        ...
        try:
            ...
            for rel_data in data.get("relationships", []):
                ...
                # Map to our IDs
                source_id = (
                    claim_id_map.get(source_orig) or
                    evid_id_map.get(source_orig) or
                    outcome_id_map.get(source_orig) or
                    damages_id_map.get(source_orig) or
                    source_orig
                )
                target_id = (
                    claim_id_map.get(target_orig) or
                    evid_id_map.get(target_orig) or
                    outcome_id_map.get(target_orig) or
                    damages_id_map.get(target_orig) or
                    target_orig
                )
    ...
    async def _extract_claim_type(
        ...
        type_keywords = {
            ...
            "DEREGULATION_CHALLENGE": ["deregulation", "decontrol", "high rent vacancy", "rent stabilized", "deregulated"],
            "HP_ACTION_REPAIRS": ["hp action", "repairs", "violations", "habitability", "housing part"],
