...
def extract_sources_from_db(graph: ArangoDBGraph) -> List[Dict[str, Any]]:
    ...
    collections = [
        "laws", "remedies", "court_cases", "legal_procedures", "damages",
        "legal_concepts", "tenant_groups", "campaigns", "tactics",
        "tenants", "landlords", "legal_services", "government_entities",
        "legal_outcomes", "organizing_outcomes", "tenant_issues", "events",
        "documents", "evidence", "jurisdictions",
    ]
...
def main():
    ...
    parser.add_argument(
        "--include-stats",
        action="store_true",
        help="Include database statistics in output"
    )
    
    parser.add_argument(
        "--pretty",
        action="store_true",
        help="Pretty-print JSON (one per line, but formatted)"
    )
