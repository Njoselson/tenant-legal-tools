...
def main():
    parser = argparse.ArgumentParser(description="Scrape CHTU resources and create manifest file")
    parser.add_argument(
        "--output",
        type=Path,
    ...
    logger.info("To ingest these resources, run:")
    logger.info(
        f"  python -m tenant_legal_guidance.scripts.ingest --deepseek-key $DEEPSEEK_API_KEY --manifest {args.output}"
    )
