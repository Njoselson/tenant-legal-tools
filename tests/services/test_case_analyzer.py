...
async def test_graph_chains_integration_with_valid_chains(mock_graph, mock_llm):
    ...
    mock_graph_chains = [
        {
            "chain": [
                ...
                {"rel": "AVAILABLE_VIA"},
                {
                    ...
                    "id": "proc1",
                    "name": "File Housing Court Complaint",
                    "cite": {"source": "guide"},
                },
                {"rel": "REQUIRES"},
                {
    ...
    result = await analyzer.analyze_case_enhanced(
        case_text="My landlord won't fix the mold in my bathroom", jurisdiction="NYC"
    )
...
async def test_graph_chains_skips_issue_with_no_chains(mock_graph, mock_llm):
    ...
    # Run analysis
    result = await analyzer.analyze_case_enhanced(
        case_text="Random text with no legal issues", jurisdiction="NYC"
    )
...
async def test_legal_elements_extraction_from_graph_chains(mock_graph, mock_llm):
    ...
    result = await analyzer.analyze_case_enhanced(
        case_text="Landlord harassing me", jurisdiction="NYC"
    )
