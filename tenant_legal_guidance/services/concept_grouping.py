...
from tenant_legal_guidance.models.entities import LegalEntity, EntityType
...
class ConceptGroupingService:
    ...
    def __init__(self, similarity_threshold: float = 0.75, min_group_size: int = 2):
        ...
        self.logger.info(f"Loaded spaCy model for concept grouping (threshold: {similarity_threshold})")
    ...
    def _find_similarity_groups(
        self, 
        entities: List[LegalEntity], 
        entity_docs: Dict[str, spacy.tokens.Doc]
        ...
        if len(entities) > 1:
            ...
            for i in range(min(3, len(entities))):
                for j in range(i + 1, min(4, len(entities))):
                    self.logger.info(f"  '{entities[i].name}' vs '{entities[j].name}': {similarity_matrix[i][j]:.3f}")
        ...
        for i in range(n):
            for j in range(i + 1, n):
                similarity = self._calculate_similarity(
                    entity_docs[entities[i].id], 
                    entity_docs[entities[j].id]
                )
    ...
    def _create_concept_group(
        ...
        for i, entity1 in enumerate(entities):
            for entity2 in entities[i+1:]:
                similarity = self._calculate_similarity(
                    entity_docs[entity1.id], 
                    entity_docs[entity2.id]
                )
    ...
    def find_similar_entities(
        ...
        candidate_entities: List[LegalEntity],
        top_k: int = 5
