...
class TestLiveExtraction:
    ...
    async def test_extraction_captures_case_details(self, case_text, deepseek_client):
        ...
        # Check outcome
        assert any(
            "dismiss" in o.name.lower() or o.disposition == "dismissed_with_prejudice"
...
class TestProofChainStructure:
    ...
    def test_deregulation_claim_has_gaps(self, expected_output):
        ...
        for pc in expected_output["proof_chains"]:
            ...
            if (
                "deregulation" in pc.get("claim_id", "").lower()
                or "decontrol" in pc.get("claim_description", "").lower()
                or "vacancy" in pc.get("claim_description", "").lower()
            ):
                chain = pc
                break
    ...
    def test_required_evidence_from_case(self, expected_output):
        ...
        assert iai_req["is_critical"] is True, "Missing this evidence caused claim to fail"
