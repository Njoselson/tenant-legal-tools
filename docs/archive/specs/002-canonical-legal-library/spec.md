...
### User Story 1 - Search Legal Sources and Add Results to Manifest (Priority: P1)

A legal researcher needs to search legal sources (case databases like Justia.com and NYSCEF Case Search, statute documentation like NYC Administrative Code on readthedocs.io, court websites) for relevant legal documents and easily add them to the manifest file for ingestion. The system should provide search tools that query these sources and export results as manifest entries.

**Why this priority**: Legal sources (case law, statutes, regulations) are primary inputs for the knowledge graph. Enabling efficient search and export streamlines the curation workflow.

**Independent Test**: Can be fully tested by searching Justia.com, NYSCEF, or NYC Admin Code for specific topics, verifying search results are returned, and confirming exported manifest entries are properly formatted.

**Acceptance Scenarios**:

1. **Given** a user wants to find cases about "rent stabilization" in New York, **When** they use the search tool with query "rent stabilization New York" on Justia.com, **Then** the system searches Justia.com and returns a list of case URLs with titles, courts, and decision dates
2. **Given** a user wants to find cases from NYS courts, **When** they use the search tool to query NYSCEF Case Search (https://iapps-train.courts.state.ny.us/nyscef/CaseSearch), **Then** the system searches NYSCEF and returns case URLs with metadata (docket numbers, courts, dates)
3. **Given** a user wants to find NYC Administrative Code sections, **When** they browse or search the NYC Admin Code documentation (https://nycadmincode.readthedocs.io/), **Then** the system can extract section URLs (e.g., Title 26 Chapter 5 on Unlawful Eviction) with metadata (document_type: statute, authority: binding_legal_authority, jurisdiction: NYC)
4. **Given** search results from any legal source are displayed, **When** a user selects documents to add, **Then** the system exports selected URLs as manifest entries with appropriate metadata (document_type, authority, jurisdiction auto-detected from URL)
5. **Given** a user wants to add a document that appears on multiple sources (e.g., same case on Justia.com and NYSCEF), **When** they add both URLs to the manifest, **Then** during ingestion the system detects they are duplicates (same content hash) and skips the duplicate with "already ingested X" log message
...
- **FR-001**: System MUST provide search/browse tools that query legal sources (Justia.com for case law, NYSCEF Case Search for court filings, NYC Administrative Code readthedocs.io for statutes, and other legal document sources) and return document URLs with metadata (title, court/section, date, docket number, jurisdiction, document type) that can be exported to manifest entries
- **FR-002**: System MUST provide tools to add manifest entries with URL validation, metadata extraction, and duplicate checking (check if URL exists in manifest or database). Manifest entries added via CLI tools may have optional `processing_status` field (defaults to "pending" or omitted); status is updated during ingestion (see Spec 006 for web UI ingestion workflow)
