...
class TestClaimExtraction:
    ...
    @pytest.mark.asyncio
    async def test_extract_claims_returns_result(self, claim_extractor, mock_llm_client, sample_case_text):
    ...
    @pytest.mark.asyncio
    async def test_extract_multiple_claims(self, claim_extractor, mock_llm_client, sample_case_text):
    ...
    @pytest.mark.asyncio
    async def test_claim_has_required_fields(self, claim_extractor, mock_llm_client, sample_case_text):
...
class TestEvidenceExtraction:
    ...
    @pytest.mark.asyncio
    async def test_extract_evidence_for_claim(self, claim_extractor, mock_llm_client, sample_case_text, sample_claim):
        ...
        evidence = await claim_extractor.extract_evidence_for_claim(
            sample_case_text, sample_claim
        )
    ...
    @pytest.mark.asyncio
    async def test_evidence_has_required_fields(self, claim_extractor, mock_llm_client, sample_case_text, sample_claim):
...
class TestOutcomeExtraction:
    ...
    @pytest.mark.asyncio
    async def test_extract_outcomes(self, claim_extractor, mock_llm_client, sample_case_text, sample_claims):
...
class TestDamagesExtraction:
    ...
    @pytest.mark.asyncio
    async def test_extract_damages(self, claim_extractor, mock_llm_client, sample_case_text, sample_outcomes):
...
class TestFullProofChainExtraction:
    ...
    @pytest.mark.asyncio
    async def test_relationships_are_correct(self, claim_extractor, mock_llm_client, sample_case_text):
        ...
        mock_llm_client.chat_completion.side_effect = [
            ...
            json.dumps({"outcomes": [{"name": "Outcome1", "type": "judgment", "disposition": "granted", "linked_claims": ["Claim1"]}]}),
            json.dumps({"damages": [{"name": "Damages1", "type": "monetary", "status": "awarded", "linked_outcome": "Outcome1"}]}),
        ]
...
class TestEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_handle_json_in_markdown(self, claim_extractor, mock_llm_client, sample_case_text):
...
'''
    ...
    async def test_parse_monetary_amounts(self, claim_extractor, mock_llm_client, sample_case_text):
        ...
        outcomes = [ExtractedOutcome(id="legal_outcome:test:0", name="Test", outcome_type="judgment", disposition="granted", description="")]
        
        mock_llm_client.chat_completion.return_value = json.dumps({
            "damages": [
                {"name": "D1", "type": "monetary", "amount": "$45,900.00", "linked_outcome": "Test"},
