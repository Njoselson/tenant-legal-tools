...
class TestEvidenceExtraction:
    ...
    async def test_extract_evidence_fallback_on_llm_failure(self, case_analyzer, mock_llm):
        ...
        case_text = "I have my lease agreement, photos of the damage, and text messages from the landlord."
...
class TestRemedyRanking:
    ...
    def test_rank_remedies_returns_top_10_only(
        self, case_analyzer, sample_chunks
    ):
...
class TestNextStepsGeneration:
    ...
    def test_generate_next_steps_limits_output(self, case_analyzer):
        ...
        evidence_gaps = {
            "needed_critical": [f"Evidence {i}" for i in range(5)]
        }
...
class TestIntegrationScenarios:
    ...
    @pytest.mark.asyncio
    async def test_complete_analysis_workflow(self, case_analyzer, mock_llm, sample_entities, sample_chunks):
        ...
        with patch.object(case_analyzer, 'retrieve_relevant_entities') as mock_retrieve:
            ...
            with patch.object(case_analyzer, '_build_sources_index') as mock_sources:
                ...
                assert result.overall_strength in ["Strong", "Moderate", "Weak", "Insufficient Data"]
