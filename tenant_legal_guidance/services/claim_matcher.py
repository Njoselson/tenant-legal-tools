...
"""
    ...
    async def match_situation_to_claim_types(
        ...
        if not all_claim_types:
            self.logger.warning(f"No claim types found in stored claims")
        ...
        if not claim_types_data:
            self.logger.warning(f"No claim types with evidence found")
            return [], []
        ...
        try:
            ...
            if start >= 0 and end > start:
                ...
                if extracted_evidence:
                    self.logger.info(f"Megaprompt extracted {len(extracted_evidence)} evidence items")
                ...
                for match_data in matched_claims:
                    ...
                    claim_type_data = next(
                        (ct for ct in claim_types_data if ct.get("canonical_name", "").upper() == canonical),
                        None
                    )
                    ...
                    for ev_assess in match_data.get("evidence_assessment", []):
                        ...
                        req_evid = next(
                            (ev for ev in required_evidence 
                             if ev.get("name") == req_evid_name),
                            None
                        )
                        if req_evid:
                            evidence_assessment.append(EvidenceMatch(
    ...
    async def _match_situation_sequential(
        ...
        for match in matched_types:
            claim_type = next(
                (ct for ct in claim_types if ct["_key"] == match["claim_type_id"]),
                None
            )
            if not claim_type:
                continue
...
"""
        ...
        try:
            ...
            if start >= 0 and end > start:
                ...
                for match in matches:
                    ...
                    claim_type = next(
                        (ct for ct in claim_types if ct.get("canonical_name", "").upper() == canonical),
                        None
                    )
                    if claim_type:
                ...
                valid_matches = [m for m in matches if "claim_type_id" in m and m["claim_type_id"]]
                return valid_matches
        except json.JSONDecodeError as e:
            self.logger.warning(f"Failed to parse LLM JSON response: {e}, response: {response[:200]}")
        except Exception as e:
    ...
    def _keyword_match_situation(
        ...
        keyword_patterns = {
            "DEREGULATION_CHALLENGE": ["deregulated", "deregulation", "decontrol", "high rent vacancy", "rent stabilized"],
            "RENT_OVERCHARGE": ["overcharge", "illegal rent", "rent too high", "rent stabilized"],
            "HP_ACTION_REPAIRS": ["repairs", "violations", "habitability", "hp action", "broken", "leak"],
...
"""
        ...
        try:
            ...
            if start >= 0 and end > start:
                ...
                for match_data in matches_data:
                    ...
                    req_evid = next(
                        (ev for ev in required_evidence if ev.get("name") == match_data["evidence_name"]),
                        None
                    )
                    if req_evid:
    ...
    def _generate_how_to_get_advice(self, evidence_name: str) -> str:
        ...
        return advice_map.get(evidence_name, f"Contact your local tenant advocacy organization for help obtaining {evidence_name}.")
    ...
    async def generate_next_steps(
        ...
        if top_match.evidence_strength == "strong":
            steps.append(f"Consider filing a {top_match.claim_type_name} claim. You have strong evidence.")
        else:
            steps.append(f"Gather missing evidence before filing {top_match.claim_type_name} claim.")
        
        # Add specific gap-filling steps
        critical_gaps = [g for g in top_match.evidence_gaps if g["is_critical"]]
        if critical_gaps:
            steps.append(f"Priority: Obtain {critical_gaps[0]['evidence_name']} - {critical_gaps[0]['how_to_get']}")
