...
### 1. Search Legal Sources and Add to Manifest

Search for legal documents from multiple sources and export to manifest:

```bash
# Search Justia.com for cases
python -m tenant_legal_guidance.scripts.search_cases \
    --source justia \
    --query "rent stabilization New York" \
    --jurisdiction "New York" \
    --output manifest_entries.jsonl

# Search NYSCEF Case Search
python -m tenant_legal_guidance.scripts.search_cases \
    --source nycef \
    --docket-number "123456/2024" \
    --output manifest_entries.jsonl

# Browse NYC Administrative Code sections
python -m tenant_legal_guidance.scripts.search_cases \
    --source nyc-admin-code \
    --title "26" \
    --chapter "5" \
    --output manifest_entries.jsonl
# This would extract URLs like https://nycadmincode.readthedocs.io/t26/c05/index.html
...
```bash
# Interactive search and add (prompts for source selection)
python -m tenant_legal_guidance.scripts.search_cases --interactive
```
