...
class CaseRelevanceFilter:
    ...
    HIGH_PRIORITY_KEYWORDS = {
        "rent stabilization", "rent stabilized", "rent control", "rent controlled",
        "rent regulated", "RSL", "rent stabilization law",
        "eviction", "non-payment proceeding", "nonpayment proceeding",
        "holdover proceeding", "summary proceeding",
        "warranty of habitability", "habitability violation", "habitable",
        "housing court", "landlord tenant court", "landlord-tenant",
        "NYCHA", "public housing", "housing authority",
        "DHCR", "division of housing and community renewal",
        "HPD", "housing preservation and development",
        "rent reduction", "rent abatement",
        "illegal eviction", "lockout", "self-help eviction",
    }
    
    # Medium-priority keywords (may be relevant)
    MEDIUM_PRIORITY_KEYWORDS = {
        "tenant", "tenancy", "rental agreement", "lease agreement",
        "landlord", "lessor", "lessee", "renter",
        "repairs", "maintenance", "heat", "hot water", "water damage",
        "harassment", "tenant harassment", "landlord harassment",
        "section 8", "housing voucher", "housing subsidy",
        "HPD violation", "code violation", "building violation",
        "rent overcharge", "overcharge", "illegal rent increase",
        "security deposit", "rent deposit",
        "possession", "ejectment", "dispossess",
        "marshal", "warrant of eviction", "eviction notice",
    }
    
    # Exclusion patterns (likely not relevant)
    EXCLUSION_PATTERNS = {
        "commercial tenant", "commercial lease", "commercial property",
        "condo", "condominium", "co-op", "cooperative",
        "foreclosure", "mortgage", "deed",
        "divorce", "matrimonial", "family court",
        "criminal court", "criminal matter", "prosecution",
        "zoning", "land use", "building permit",
    }
    ...
    def keyword_filter(
        ...
        combined_text = " ".join(filter(None, [case_name, court or "", text_snippet or "", url or ""]))
...
"""
        ...
        try:
            ...
            # Parse response
            decision_match = re.search(r'DECISION:\s*(RELEVANT|NOT RELEVANT)', response, re.IGNORECASE)
    ...
    async def filter_case(
        ...
        if use_llm and self.llm_client:
            ...
            llm_result = await self.llm_filter(
                case_name, court, decision_date, text_snippet
            )
