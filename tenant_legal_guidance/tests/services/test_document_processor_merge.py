...
from tenant_legal_guidance.models.entities import LegalEntity, EntityType, SourceType, SourceMetadata
from tenant_legal_guidance.services.document_processor import DocumentProcessor


def make_entity(eid: str, etype: EntityType, name: str, desc: str = "", jurisdiction: str | None = None):
    attrs = {}
    if jurisdiction:
...
def test_deduplicate_entities_simple():
    ...
    e2 = make_entity("law:rent_stabilization_dup", EntityType.LAW, "Rent Stabilization", desc="NYC law")
...
async def test_semantic_merge_entities_thresholds(monkeypatch):
    ...
    kg = MagicMock()
    kg.search_entities_by_text.return_value = [Cand("law:housing_part_action", "Housing Part Action", "HP action in housing court")]
...
def test_update_relationship_references_skips_self_edges():
    ...
    rels = [
        ...
        LegalRelationship(source_id="c", target_id="c", relationship_type=RelationshipType.REQUIRES),
    ]
    mapping = {"a": "x", "b": "x", "c": "c"}
